...
import { isSameWeek, isThisWeek } from "date-fns";
...
const Statistics: FC<StatisticsProps> = ({ bookings }) => {
  ...
  useEffect(() => {
    if (bookings.length) {
      setThisWeek(bookings.filter((booking) => isThisWeek(booking.dateTime)));
      setPrevWeek(
        bookings.filter((booking) =>
          isSameWeek(booking.dateTime, getLastWeeksDate())
        )
      );
    }
  }, [bookings]);
          ...
          <div className="-mx-3 mt-0 flex flex-wrap">
            <div className="mt-0 w-1/4 max-w-full flex-none py-4 pl-0 pr-3">
              <div className="mb-2 flex">
                <div className="shadow-soft-2xl mr-2 flex h-5 w-5 items-center justify-center rounded bg-gradient-to-tl from-purple-700 to-pink-500 bg-center fill-current text-center text-neutral-900">
                  <svg
                    width="10px"
                    height="10px"
                    viewBox="0 0 40 44"
